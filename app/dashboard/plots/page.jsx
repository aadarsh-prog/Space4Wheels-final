  ...
  navigator.geolocation.getCurrentPosition(
    (position) => {
      ...
      if (typeof destination === 'object') {
        // Try different property names and structures
        if (destination.lat !== undefined && destination.lng !== undefined) {
          destLat = destination.lat;
          destLng = destination.lng;
        } 
        else if (destination.latitude !== undefined && destination.longitude !== undefined) {
          destLat = destination.latitude;
          destLng = destination.longitude;
        }
        // Check for nested location object
        else if (destination.location && typeof destination.location === 'object') {
          if (destination.location.lat !== undefined && destination.location.lng !== undefined) {
            destLat = destination.location.lat;
            destLng = destination.location.lng;
          } else if (destination.location.latitude !== undefined && destination.location.longitude !== undefined) {
...
export default function FindParkingPage() {
 ...
 <Button
  ...
  className="w-full"
  onClick={(e) => {
    e.stopPropagation(); // Stop click from propagating to parent
    
    // Use hardcoded coordinates instead of plot.location
    const hardcodedLocation = {
      lat: 22.719568, 
      lng: 75.857727 // These are the coordinates you mentioned in your code
    };
    
    // Call the function with hardcoded coordinates
    navigateToLocation(hardcodedLocation);
  }}
>
  Navigate
</Button>
